...
import { mtndew1, mtndew2, mtndew3, mtndew4, mtndew5, mtndew6 } from "../public/mtndew";
import { mediaone1, mediaone2, mediaone3, mediaone4, mediaone5, mediaone6 } from "../public/mediaone";
import { waveform1, waveform2, waveform3, waveform4, waveform5, waveform6 } from "../public/waveform";
...
const SubMainContent = () => {
  ...
  const subPhotos = {
    ...
    'Media One': [mediaone1, mediaone2, mediaone3, mediaone4, mediaone5, mediaone6],
    'Waveform': [waveform1, waveform2, waveform3, waveform4, waveform5, waveform6],
    ...
    Array(5)
      ...
      .map((_e, i) => (
        <>
          <div
            key={`sampleCard-${i}`}
            className={`project-card relative ${project === projectNames[i + 1][0] ? "h-[500px] w-[1200px]" : "h-[340px] w-[540px]"} mr-[70px] shrink-0`}>
            <div 
              className={`img-box ${project === projectNames[i + 1][0] ? "hide-project" : ""}`} 
            ...
            <div  className="project-sub-photos">
              <div className="gallery">
                {subPhotos[projectNames[i + 1][0]].map((photo, j) => {
                  return (
                    <img className={`${project === projectNames[i + 1][0] ? `animate glow delay-${j + 1}` : ""}`} src={photo} alt={`${project}-${j}`} />
                  )
                })}
                
              </div>
            </div>
            <div className={`${project === projectNames[i + 1][0] ? "project-sub-photos-overlay" : ""}`} onClick={() => setProject('')}></div>
          </div>
        </>
      ))
  );
        ...
        <div className="info-section-container flex">
          <div className="main-text-container h-[300px] w-[760px] relative">
            ...
            <div
              ...
              <a className='btn' href="https://drive.google.com/file/d/1mMQBAZBl0QM2-D7IW6e3vz3KP_7GuA0X/view?usp=sharing" rel="noreferrer" target="_blank">My Resume</a>
            </div>
          </div>
